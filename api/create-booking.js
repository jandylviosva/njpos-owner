...
async function sendCreationEmail({ requiresPayment, booking, storeName, storeAddress, businessPhone, bookingNoun, payUrl }) {
  ...
  const html = `<div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;padding:24px;background:#f9fafb">
    ...
    <div style="background:#fff;border-radius:12px;padding:24px;border:1px solid #e5e7eb">
      <div style="color:${requiresPayment?"#b45309":"#0d9488"};font-weight:700;font-size:13px;margin-bottom:10px">${requiresPayment?"⏳ Payment Needed":"✓ "+noun+" Confirmed"}</div>
      ...
      ${requiresPayment ? `<a href="${payUrl}" style="display:block;text-align:center;background:#0d9488;color:#fff;text-decoration:none;font-weight:700;font-size:14px;padding:12px 0;border-radius:8px;margin-top:18px">Pay Now</a>` : ""}
...
export default async function handler(req, res) {
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    ...
    if (patched.length > 0) {
      const origin = req.headers.origin && ALLOWED_ORIGINS.includes(req.headers.origin) ? req.headers.origin : "https://client.pospro-portal.com";
      sendCreationEmail({
